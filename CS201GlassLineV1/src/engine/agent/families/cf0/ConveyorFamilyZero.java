package engine.agent.families.cf0;
...
public class ConveyorFamilyZero extends Agent implements ConveyorFamilyInterface
{
	//DATA
	ConveyorFamilyInterface nextNeighbor;
	...
	//CONSTRUCTOR
	public ConveyorFamilyZero(String nombre, Transducer tdeuce)
	{
		super(nombre, tdeuce);
	...
	public boolean pickAndExecuteAnAction() 
	{
		...
		{
			//Turn the conveyor off at the last possible moment for V2
		...
		if(!conveyorFlagged && !beltDisabled && !stationDisabled)
		{
			...
			if(popupLoaded)
			{
				...
				if(!glassList.get(0).glass.getRecipe().getNeedCutting())
				{
					ReleaseGlass();
					return true;
				}
				
				if(glassList.get(0).glass.getRecipe().getNeedCutting() && !machining && glassList.get(0).machined)
				{
					ReleaseGlass();
					return true;
				}
				
				if(glassList.get(0).glass.getRecipe().getNeedCutting() && !machining)
				{
	...
	private void ReleaseGlass()
	{
		...
		transducer.fireEvent(TChannel.CUTTER, TEvent.WORKSTATION_RELEASE_GLASS, channelWorkStation);
	...
	private void MachineGlass()
	{
		...
		//fire the transducer event
		transducer.fireEvent(TChannel.CUTTER, TEvent.WORKSTATION_DO_ACTION, channelWorkStation);
	}
	...
	public synchronized void eventFired(TChannel channel, TEvent event, Object[] args) 
	{
		...
		{
			if(event == TEvent.SET_RECIPE)
			{
				Glass temp;
				temp = (Glass)args[0];
				this.msgHereIsGlass(temp);
			}	
		}
