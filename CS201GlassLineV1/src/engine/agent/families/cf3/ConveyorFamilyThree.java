package engine.agent.families.cf3;
...
public class ConveyorFamilyThree extends Agent implements ConveyorFamilyInterface
{
	//DATA
	...
	//CONSTRUCTOR
	public ConveyorFamilyThree(String nombre, Transducer tdeuce)
	{
		super(nombre, tdeuce);
	...
	public boolean pickAndExecuteAnAction() 
	{
		...
		if(!conveyorFlagged && !beltDisabled && !stationDisabled)
		{
			...
			if(popupLoaded)
			{
				...
				if(!glassList.get(0).glass.getRecipe().getNeedManualBreakout())
				{
					ReleaseGlass();
					return true;
				}
				
				if(glassList.get(0).glass.getRecipe().getNeedManualBreakout() && !machining && glassList.get(0).machined)
				{
					ReleaseGlass();
					return true;
				}
				
				if(glassList.get(0).glass.getRecipe().getNeedManualBreakout() && !machining)
				{
	...
	private void ReleaseGlass()
	{
		...
		transducer.fireEvent(TChannel.MANUAL_BREAKOUT, TEvent.WORKSTATION_RELEASE_GLASS, channelWorkStation);
	...
	private void MachineGlass()
	{
		...
		//fire the transducer event
		transducer.fireEvent(TChannel.MANUAL_BREAKOUT, TEvent.WORKSTATION_DO_ACTION, channelWorkStation);
	}
