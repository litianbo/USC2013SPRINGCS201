...
public class ControlPanel extends JPanel implements TReceiver
{
	...
	public ControlPanel()
	{
		...
		selectTabbedPanel.setPreferredSize(new Dimension(400, 300));
		selectTabbedPanel.setMaximumSize(new Dimension(400, 300));
		selectTabbedPanel.add("Order Glass", glassSelectPanel);
		selectTabbedPanel.add("Operator Messages", tracePanel);
	...
	public ControlPanel(FactoryPanel fPanel, Transducer fTransducer)
	{
		...
		nonNormPanel.setAllTransducer(fTransducer);
		tracePanel.setTransducer(fTransducer);
	...
	public synchronized void eventFired(TChannel channel, TEvent event, Object[] args)
	{
		if (channel == TChannel.SENSOR && event == TEvent.SENSOR_GUI_RELEASED) {
			...
			if (sensorNumber == 0) {
				glassSelectPanel.reenableOrderButton();
			}
		}
	}
