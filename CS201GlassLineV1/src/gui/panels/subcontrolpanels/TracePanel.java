...
public class TracePanel extends JPanel implements ActionListener, TReceiver
{
	...
	public void setTransducer(Transducer transducer)
	{
		...
		myTransducer.register(this, TChannel.CROSS_SEAMER);
		myTransducer.register(this, TChannel.GRINDER);
	}
	
	@Override
	public void eventFired(TChannel channel, TEvent event, Object[] args) 
	{
		if (event == TEvent.OPERATOR_NEEDED){
			if ((Integer)args[0] == 0){
				if (channel == TChannel.DRILL){
					addPanelMessage("Top Drill workstation requesting operator.\n", null);
				}
				else if (channel == TChannel.CROSS_SEAMER){
					addPanelMessage("Top Cross-seamer workstation requesting operator.\n", null);
				}
				else if (channel == TChannel.GRINDER){
					addPanelMessage("Top Grinder workstation requesting operator.\n", null);
				}
				
			}
			else{
				if (channel == TChannel.DRILL){
					addPanelMessage("Bottom Drill workstation requesting operator.\n", null);
				}
				else if (channel == TChannel.CROSS_SEAMER){
					addPanelMessage("Bottom Cross-seamer workstation requesting operator.\n", null);
				}
				else if (channel == TChannel.GRINDER){
					addPanelMessage("Bottom Grinder workstation requesting operator.\n", null);
				}			
			}
		}
	}
