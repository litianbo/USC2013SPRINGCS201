...
public class DisplayPanel extends JLayeredPane {
	...
	/**
	 * Defining different layers: 1- Background Image layer = layer 0 2- the
	 * layer that all fixed machinery are at = layer 1 3- the layer where kits
	 * are at = layer 2 4- the layer where parts are at normally = layer 3 5-
	 * the layer where kits are at in movement= layer 4 6- the layer where parts
	 * are in kit that moves = layer 5 7- the layer where kit robot is at =
	 * layer 6 8- the layer where bins are at = layer 7 9- the layer where
	 * gantry robot operates = layer 8 10- the layer where part robot operates
	 * and temporary layer for parts on robot = layer 9 11- Ceiling layer =
	 * layer 10 12- Drag and drop layer = layer 11
	 * 
	 */
	public final static Integer BG_LAYER = 0, STATIONARY_MACHINERY_LAYER = 1,
			KIT_LAYER = 2, PART_NORMAL_LAYER = 3, KIT_IN_MOVE_LAYER = 4,
			PART_IN_MOVE_LAYER = 5, KITBOT_LAYER = 6, BIN_LAYER = 1,
			GANTRY_LAYER = 8, PARTBOT_LAYER = 9, CEILING_LAYER = 10,
			DND_LAYER = 11,

			GROUND_LAYER = 0, CONVEYOR_LAYER = 2, SENSOR_LAYER = 7,
			MACHINE_LAYER = 12, POPUP_LAYER = 13, PART_LAYER = 15,
			ROBOT_LAYER = 19, PART_POPUP_LAYER = 20, ONLINE_MACHINE_LAYER = 25,
			ROOF_LAYER = 100;
	...
	public void setupComponents() {
		...
		createOnlineComponent(
				currentComponent.getX()
						- (int) (ImageIcons.getIconList("cutter").get(0)
								.getIconWidth() / 2),
				currentComponent.getCenterY(), MachineType.CUTTER,
				ConveyorDirections.LEFT);

		createConveyor(currentComponent.getX()
				- (int) (GUIConveyor.conveyorRight.getIconWidth() / 2),
				currentComponent.getCenterY(), ConveyorDirections.LEFT,
				numConveyors++);

		createTransfer(currentComponent.getX()
				- (int) (GUIShuttle.leftConnector.getIconWidth() / 2),
				currentComponent.getCenterY(), ConveyorDirections.UP);

		// UP

		createConveyor(currentComponent.getCenterX(), currentComponent.getY()
				- (int) (GUIConveyor.conveyorUp.getIconHeight() / 2),
				ConveyorDirections.UP, numConveyors++);

		createBreakout(currentComponent.getCenterX(), currentComponent.getY()
				- (int) (ImageIcons.getIconList("breakOut").get(0)
						.getIconHeight() / 2));
		...
		createManualBreakout(
				currentComponent.getCenterX(),
				currentComponent.getY()
						- (int) (ImageIcons.getIconList("manualBreakout")
								.get(0).getIconHeight() / 2));

		createConveyor(currentComponent.getCenterX(), currentComponent.getY()
				- (int) (GUIConveyor.conveyorUp.getIconHeight() / 2),
				ConveyorDirections.UP, numConveyors++);

		createTransfer(currentComponent.getCenterX(), currentComponent.getY()
				- (int) (GUIShuttle.leftConnector.getIconHeight() / 2),
				ConveyorDirections.RIGHT);

		// RIGHT

		createConveyor(currentComponent.getX()
				+ currentComponent.getIcon().getIconWidth()
				+ (int) (GUIConveyor.conveyorRight.getIconWidth() / 2),
				currentComponent.getCenterY(), ConveyorDirections.RIGHT,
				numConveyors++);

		createPopUp(currentComponent.getX()
				+ currentComponent.getIcon().getIconWidth()
				+ (int) (GUIShuttle.leftConnector.getIconHeight() / 2) - 5,
				currentComponent.getCenterY());

		createOfflineComponent(currentComponent.getCenterX(),
				currentComponent.getY()
						- (int) (ImageIcons.getIconList("drill").get(0)
								.getIconHeight() / 2), MachineType.DRILL, 1);
		createOfflineComponent(currentComponent.getCenterX(),
				currentComponent.getY()
						+ currentComponent.getIcon().getIconHeight()
						+ (int) (ImageIcons.getIconList("drill").get(0)
								.getIconHeight() / 2), MachineType.DRILL, 2);
		...
		createOfflineComponent(
				currentComponent.getCenterX(),
				currentComponent.getY()
						- (int) (ImageIcons.getIconList("crossSeamer").get(0)
								.getIconHeight() / 2),
				MachineType.CROSS_SEAMER, 1);
		createOfflineComponent(currentComponent.getCenterX(),
				currentComponent.getY()
						+ currentComponent.getIcon().getIconHeight()
						+ (int) (ImageIcons.getIconList("crossSeamer").get(0)
								.getIconHeight() / 2),
				MachineType.CROSS_SEAMER, 2);
		...
		createOfflineComponent(
				currentComponent.getCenterX(),
				currentComponent.getY()
						- (int) (ImageIcons.getIconList("grinder").get(0)
								.getIconHeight() / 2), MachineType.GRINDER, 1);
		createOfflineComponent(currentComponent.getCenterX(),
				currentComponent.getY()
						+ currentComponent.getIcon().getIconHeight()
						+ (int) (ImageIcons.getIconList("grinder").get(0)
								.getIconHeight() / 2), MachineType.GRINDER, 2);
		...
		createOnlineComponent(currentComponent.getX()
				+ currentComponent.getIcon().getIconWidth()
				+ (int) (ImageIcons.getIconList("washer").get(0)
						.getIconHeight() / 2), currentComponent.getCenterY(),
				MachineType.WASHER, ConveyorDirections.RIGHT);
		...
		createTransfer(currentComponent.getX()
				+ currentComponent.getIcon().getIconWidth()
				+ (int) (GUIShuttle.leftConnector.getIconHeight() / 2),
				currentComponent.getCenterY(), ConveyorDirections.DOWN);

		// DOWN

		createConveyor(currentComponent.getCenterX(), currentComponent.getY()
				+ currentComponent.getIcon().getIconHeight()
				+ (int) (GUIConveyor.conveyorDown.getIconHeight() / 2),
				ConveyorDirections.DOWN, numConveyors++);

		createOnlineComponent(currentComponent.getCenterX(),
				currentComponent.getY()
						+ currentComponent.getIcon().getIconHeight()
						+ (int) (ImageIcons.getIconList("paint").get(0)
								.getIconHeight() / 2), MachineType.PAINT,
				ConveyorDirections.DOWN);
		...
		createOnlineComponent(currentComponent.getCenterX(),
				currentComponent.getY()
						+ currentComponent.getIcon().getIconHeight()
						+ (int) (ImageIcons.getIconList("paint").get(0)
								.getIconHeight() / 2), MachineType.UV_LAMP,
				ConveyorDirections.DOWN);
		...
		createOnlineComponent(
				currentComponent.getX()
						- (int) (ImageIcons.getIconList("oven").get(0)
								.getIconWidth() / 2),
				currentComponent.getCenterY(), MachineType.OVEN,
				ConveyorDirections.LEFT);
		...
		createTruck(
				currentComponent.getX()
						- (int) (ImageIcons.getIconList("truck").get(0)
								.getIconWidth() / 2),
				currentComponent.getCenterY());

		/*
		 * NOTE: You can observe the factory work on one piece of glass without
		 * agents by leaving the below line alone. Comment it out when you are
	...
	private void createOfflineComponent(int x, int y, MachineType type,
			int machineNumber) {
		GUIComponentOffline offlineComponent = new GUIComponentOffline(type,
				transducer);
		offlineComponent.setParent(this);
		offlineComponent.setName(type.toString().toLowerCase() + " "
				+ machineNumber);
		...
		((GUIPopUp) currentComponent).setPairedMachine(offlineComponent,
				machineCount);
		machineCount = ++machineCount % 2;
	}

	private void createOnlineComponent(int x, int y, MachineType type,
			ConveyorDirections direction) {
		GUIComponentOnline onlineComponent = new GUIComponentOnline(type,
				transducer);
	...
	private void createConveyor(int x, int y, ConveyorDirections direction,
			int nextIndex) {
		GUIConveyor conveyor = new GUIConveyor(this, direction, 3, 7,
				nextIndex, transducer);
		...
		if (direction == ConveyorDirections.UP) {
			createSensor(conveyor.getCenterX(), conveyor.getCenterY()
					+ (2 * conveyor.getIcon().getIconHeight()) / 5,
					(conveyorCount) * 2);
			createSensor(conveyor.getCenterX(), conveyor.getCenterY()
					- (2 * conveyor.getIcon().getIconHeight()) / 5,
					(conveyorCount) * 2 + 1);
			...
			createSensor(conveyor.getCenterX()
					+ (2 * conveyor.getIcon().getIconWidth()) / 5,
					conveyor.getCenterY(), (conveyorCount) * 2);
			createSensor(conveyor.getCenterX()
					- (2 * conveyor.getIcon().getIconWidth()) / 5,
					conveyor.getCenterY(), (conveyorCount) * 2 + 1);
