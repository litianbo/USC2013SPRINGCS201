...
public class GUITruck extends GuiComponent {
	...
	private int TRUCK_SPEED = 10;// added by monroe. default was 2 before, and
									// hard-coded everywhere.

	private int MAX_TRUCK_SPEED = 100;// added by monroe.
	private int MIN_TRUCK_SPEED = 1;// added by monroe.

	boolean breakDown = false;

	/**
	 * Allows changes to be made mid-program to GUITruck speed (perhaps a GUI
	 * button).
	 * 
	 * @author Monroe
	 */
	...
	/**
	 * Moving the stack of glass to and from the truck to the bin, depending on
	 * whether glass is being taken from bin or bin is being refilled
	 */
	public void movePartIn() {
		...
		if (part.getCenterY() == getCenterY()) {
			// part.setVisible(false);//monroe moved to after the part is
			// completely removed
			// part = null;//monroe moved to after the part is completely
			// removed
			state = TruckState.IDLE;
			transducer.fireEvent(TChannel.TRUCK,
					TEvent.TRUCK_GUI_LOAD_FINISHED, null);
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (state == TruckState.RETURNING && !breakDown) {
			moveTruckIn();
		}
	}
	...
	private void moveTruckOut() {
		...
		part.setLocation((int) part.getLocation().getX() + TRUCK_SPEED,
				(int) part.getLocation().getY());// added by monroe to show part
													// leaving on truck
		if (getCenterX() > (parent./* getParent().getGuiParent(). */getWidth() + this
				.getWidth() * 3 / 4))// changed from < to > by monroe and added
										// math to make truck actually leave the
										// right panel
		{
	...
	private void moveTruckIn() {
		setCenterLocation(getCenterX() - TRUCK_SPEED, getCenterY());// changed
																	// from -1
																	// to
																	// -TRUCK_SPEED
																	// by monroe
		...
		{
			setCenterLocation(((int) truckOrig.getX()),
					((int) truckOrig.getY()));
			state = TruckState.LOADING;
			transducer.fireEvent(TChannel.TRUCK,
					TEvent.TRUCK_GUI_EMPTY_FINISHED, null);
		}
	}
