...
public class GUIComponentOffline extends GuiAnimationComponent implements ActionListener, Serializable
{
	...
	public void doAnimate()
	{
		...
		else if (counter >= imageicons.size() && numAnimation+1 < maxNumAnimation){
			counter = 0;
			numAnimation++;
		}
		else if (counter >= imageicons.size() && numAnimation+1 >= maxNumAnimation)
		{
			if (!glassBroken){
				...
				args[0] = index;
				animationState = GuiAnimationComponent.AnimationState.DONE;
				transducer.fireEvent(channel, TEvent.WORKSTATION_REMOVE_GLASS, args);
				removeGlass();
			}
		}
	}
	...
	{
		if (event == TEvent.CHANGE_WORKSTATION_SPEED){
			maxNumAnimation = (Integer)args[0];
		}
